...
class SimulatedBinaryCrossoverTest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        ...
        crossover = SimulatedBinaryCrossover(0.5, distributionIndex, variableBounds, rng);
    }
    ...
    double Beta1(const SimulatedBinaryCrossover& crossover, double x1, double x2, double lowerBound) {
        return crossover.Beta1(x1, x2, lowerBound);
    }

    double Beta2(const SimulatedBinaryCrossover& crossover, double x1, double x2, double upperBound) {
...
TEST_F(SimulatedBinaryCrossoverTest, PerformCrossover) {
    ...
    for (int i = 0; i < child.solution.size(); i++) {
        if (i % 2 == 0) {
            double beta = Beta1(crossover, parent1[i], parent2[i], variableBounds[i].first);
            double alpha = Alpha(crossover, beta);
            double betaq = std::pow(0.2 * alpha, 1.0 / (distributionIndex + 1.0));
...
TEST_F(SimulatedBinaryCrossoverTest, Betaq) {
    ...
    EXPECT_DOUBLE_EQ(Betaq(crossover, alpha), std::pow(0.5, 1.0 / (distributionIndex + 1.0)));
    ...
    EXPECT_DOUBLE_EQ(Betaq(crossover, alpha), std::pow(1.0 / (2.0 - 0.5 * alpha), 1.0 / (distributionIndex + 1.0)));

    EXPECT_CALL(*rng, Random()).WillOnce(Return(1.0));
    EXPECT_DOUBLE_EQ(Betaq(crossover, 1.21), std::pow(1.0 / 0.79, 1.0 / (distributionIndex + 1.0)));
}

TEST_F(SimulatedBinaryCrossoverTest, Alpha) {
    EXPECT_DOUBLE_EQ(Alpha(crossover, 0.5), 2.0 - std::pow(0.5, -(distributionIndex + 1.0)));
    EXPECT_DOUBLE_EQ(Alpha(crossover, 1.0), 1.0);
}

TEST_F(SimulatedBinaryCrossoverTest, Beta1) {
    EXPECT_DOUBLE_EQ(Beta1(crossover, 0.34, 0.82, 0.0), 1.0 + 2.0 * 0.34 / 0.48);
}

TEST_F(SimulatedBinaryCrossoverTest, Beta2) {
    EXPECT_DOUBLE_EQ(Beta2(crossover, 0.34, 0.82, 1.0), 1.0 + 2.0 * 0.18 / 0.48);
}
