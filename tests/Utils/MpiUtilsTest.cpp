...
TEST(MpiUtilsTest, CalculateNodeWorkload) {
    ...
    for (int i = 0; i < ranks.size(); i++) {
        int actual =
            CalculateNodeWorkload(totalPopulationSize, ranks[i], parallelSize);
...
TEST(MpiUtilsTest, CalculateNodeStartIndex) {
    ...
    for (int i = 0; i < ranks.size(); i++) {
        int actual = CalculateNodeStartIndex(totalPopulationSize, ranks[i],
                                             parallelSize);
        EXPECT_EQ(actual, expected[i]);
    }
}
...
TEST(MpiUtilsTest, GenerateDataCountsAndDisplacements) {
    ...
    auto actual = GenerateDataCountsAndDisplacements(nodeWorkloads, dataSize,
                                                     parallelSize);
