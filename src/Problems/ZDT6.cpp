...
double ZDT6::F1(double x1) const {
    return 1.0 - std::exp(-4.0 * x1) *
                     std::pow(std::sin(6.0 * std::numbers::pi * x1), 6.0);
}
...
double ZDT6::G(const Eigen::ArrayXd& X) const {
    ...
    return 1.0 +
           9.0 * std::pow(
                     sum / (static_cast<double>(DecisionVariablesNum()) - 1.0),
                     0.25);
}
...
std::vector<Eigen::ArrayXd> ZDT6::GenerateParetoFront(int pointsNum) const {
    ...
    std::vector<double> x =
        Ranged(region.first, region.second,
               (region.second - region.first) / (pointsNum - 1));
