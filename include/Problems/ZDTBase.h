...
class ZDTBase : public IProblem<double>, public IBenchmark {
   public:
    ...
    int DecisionVariablesNum() const override { return decisionVariablesNum; }
    int ObjectivesNum() const override { return objectivesNum; }
    const std::vector<std::pair<double, double>>& VariableBounds()
        const override {
        return variableBounds;
    }
    double GOfParetoFront() const { return gOfParetoFront; }
