...
class NewMoead : public IMoead<DecisionVariableType> {
   ...
   public:
    NewMoead(int generationNum, int neighborhoodSize, int divisionsNumOfWeightVector, int migrationInterval,
             std::string neighboringMigrationTargetFilePath, std::string idealPointMigrationTargetFilePath,
        ...
        : generationNum(generationNum),
          ...
          neighboringMigrationTargetFilePath(neighboringMigrationTargetFilePath),
   ...
   protected:
    ...
    void InitializeIsland() {
        ...
        if (rank == 0) {
            CalculateDestinationRanks(neighborhoodIndexes1d, populationSizes, sendDestinationRanksByRank,
                                      destinationRanksByRankSizes);
        }
        auto ranksToSend = Scatterv(sendDestinationRanksByRank, destinationRanksByRankSizes, 1, rank, parallelSize);
        ...
        auto receivedExternalSolutions = ScatterPopulation(ranksToSend, neighboringRanks);
    ...
    void ReadMigrationTargetFile() {
        std::ifstream neighboringMigrationTargetFile(neighboringMigrationTargetFilePath);
        std::ifstream idealPointMigrationTargetFile(idealPointMigrationTargetFilePath);

        if (!neighboringMigrationTargetFile) {
            throw std::runtime_error("Failed to open neighboring migration target file");
        }

        if (!idealPointMigrationTargetFile) {
            throw std::runtime_error("Failed to open ideal point migration target file");
        }

        neighboringMigrationTarget = InitializeMigrationTarget(neighboringMigrationTargetFile);
        idealPointMigrationTarget = InitializeMigrationTarget(idealPointMigrationTargetFile);
    }
    ...
    void CalculateDestinationRanks(const std::vector<int>& neighborhoodIndexes, const std::vector<int>& populationSizes,
                                   std::vector<int>& outDestinationRanksByRank, std::vector<int>& outSizes) {
        std::vector<std::set<int>> destinationRanksByRank(parallelSize, std::set<int>());
    ...
    std::vector<std::vector<double>> ScatterPopulation(const std::vector<int>& destinationRanks,
                                                       const std::set<int>& recipients) {
        std::vector<double> individualsToSend;
        individualsToSend.reserve(isIdealPointMigration ? internalIndexes.size() * singleMessageSize + objectivesNum
        ...
        for (std::size_t i = 0; i < destinationRanks.size(); ++i) {
            MPI_Isend(individualsToSend.data(), individualsToSend.size(), MPI_DOUBLE, destinationRanks[i], MessageTag,
    ...
    void InitializePopulation() {
        ...
        auto sampledIndividuals = sampling->Sample(sampleNum, decisionVariablesNum);
    ...
    void InitializeExternalIndividuals(const std::vector<std::vector<double>>& receivedIndividuals,
                                       std::unordered_map<int, Individual<DecisionVariableType>>& inputIndividuals) {
        for (auto&& receive : receivedIndividuals) {
            int limit = isIdealPointMigration ? receive.size() - objectivesNum : receive.size();
            for (int i = 0; i < limit; i += singleMessageSize) {
                ...
                inputIndividuals[index].solution =
                    Eigen::Map<const Eigen::ArrayXd>(receive.data() + (i + 1), decisionVariablesNum);
                inputIndividuals[index].objectives =
                    Eigen::Map<const Eigen::ArrayXd>(receive.data() + (i + 1 + decisionVariablesNum), objectivesNum);
    ...
    void InitializeIndividualAndWeightVector(std::vector<Eigen::ArrayXd>& weightVectors,
        ...
        for (int i = 0; i < externalIndexes.size(); i++) {
            individuals[externalIndexes[i]].weightVector = std::move(externalWeightVectors[i]);
        }
    }
    ...
    void UpdateIndividual(int index, Individual<DecisionVariableType>& newIndividual) {
    ...
    std::unordered_map<int, std::vector<double>> CreateMessages() {
        ...
        updatedIndividuals.reserve(updatedSolutionIndexes.size() * singleMessageSize);
        for (auto&& i : updatedSolutionIndexes) {
            updatedIndividuals.push_back(i);
            updatedIndividuals.insert(updatedIndividuals.end(), individuals[i].solution.begin(), individuals[i].solution.end());
            updatedIndividuals.insert(updatedIndividuals.end(), individuals[i].objectives.begin(),
                                      individuals[i].objectives.end());
        }
    ...
    void SendMessages() {
        // MPI_Isendで使うバッファ
        static std::array<std::unordered_map<int, std::vector<double>>, maxBufferSize> messageToSendBuffers;
        static int messageToSendBufferIndex = 0;
        messageToSendBufferIndex = (messageToSendBufferIndex + 1) % maxBufferSize;

        std::unordered_map<int, std::vector<double>>& messagesToSend = messageToSendBuffers[messageToSendBufferIndex];
    ...
    std::vector<std::vector<double>> ReceiveMessages() {
        ...
        receivedMessages.reserve(neighboringMigrationTarget.size() * migrationInterval);
    ...
    void IprobeAndRecvLoop(int source, std::vector<std::vector<double>>& outReceivedMessages) {
        while (true) {
            MPI_Status status;
            int canReceive;
            ...
            MPI_Recv(receive.data(), receivedDataSize, MPI_DOUBLE, source, MessageTag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    void NeighboringMigration(std::vector<double>& message) {
        ...
        bool containsIdealPoint = isIdealPointMigration && message.size() % singleMessageSize == objectivesNum;
    ...
    void IdealPointMigration(std::vector<double>& message) {
        if (message.size() == objectivesNum) {
            Eigen::ArrayXd receivedIdealPoint = Eigen::Map<Eigen::ArrayXd>(message.data(), objectivesNum);
            UpdateIdealPoint(receivedIdealPoint);
        }
    }
