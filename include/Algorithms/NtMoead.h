...
class NtMoead : public IMoead<DecisionVariableType> {
   public:
    ...
    NtMoead(int generationNum, int neighborhoodSize,
   ...
   private:
    ...
    void CalculateRankIndexesByNode(
        const std::vector<std::vector<int>>& noduplicateIndexes,
    ...
    void CalculateRankIndexesToSend(std::vector<int>& allRankIndexes,
                                    std::vector<int>& numsRankIndexes);
    std::vector<std::vector<int>> CalculateNoduplicateIndexes(
...
template <typename DecisionVariableType>
std::vector<Eigen::ArrayXd> NtMoead<DecisionVariableType>::GetObjectivesList()
    const {
...
NtMoead<DecisionVariableType>::GetSolutionList() const {
...
void NtMoead<DecisionVariableType>::InitializeIsland() {
    ...
    if (rank == 0) {
        ...
        noduplicateIndexes = CalculateNoduplicateIndexes(
            neighborhoodIndexes1d, populationSizes, allNodeIndexes);
    }
    ...
    if (rank == 0) {
        CalculateRankIndexesByNode(noduplicateIndexes, allRankIndexes,
                                   sizesAllRankIndexes, numsRankIndexes,
                                   sizesNumRank);
    }
    std::vector<int> receivedRankIndexes =
        Scatterv(allRankIndexes, sizesAllRankIndexes, 1, rank, parallelSize);
    std::vector<int> receivedNumsRankIndexes =
        Scatterv(numsRankIndexes, sizesNumRank, 1, rank, parallelSize);
    ...
    std::vector<int> ranksToSend = Scatterv(
        sendRanksToSentByRank, ranksToSentByRankSizes, 1, rank, parallelSize);
    std::set<int> neighboringRanks = CalculateNeighboringRanks();

    auto receivedExternalIndividuals =
...
NtMoead<DecisionVariableType>::ReadAdjacencyList() {
...
/// @param outAllRankIndexes {{rank1, index1, index2, rank2, index3, ...},
/// {rank3, index4, ...}, ...}
/// @param outSizesAllRankIndexes {allSizeOfNode1, allSizeOfNode2, ...}
/// @param outNumsRankIndexes {{numOfRank1, numOfRank2, ...}, {numOfRank3, ...},
/// ...}
/// @param outSizesNumRank {sizeOfNumOfNode1, sizeOfNumOfNode2, ...} equal to
/// degrees of nodes
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::CalculateRankIndexesByNode(
    const std::vector<std::vector<int>>& noduplicateIndexes,
    std::vector<int>& outAllRankIndexes,
    std::vector<int>& outSizesAllRankIndexes,
    ...
    for (int rank = 0; rank < adjacencyList.size(); rank++) {
        ...
        for (auto&& neighbor : adjacencyList[rank]) {
            ...
            individualIndexes.insert(noduplicateIndexes[neighbor].begin(),
                                     noduplicateIndexes[neighbor].end());
            for (auto&& k : adjacencyList[neighbor]) {
                if (k == rank) {
                    continue;
                }

                individualIndexes.insert(noduplicateIndexes[k].begin(),
                                         noduplicateIndexes[k].end());
            }

            outAllRankIndexes.push_back(neighbor);
            outAllRankIndexes.insert(outAllRankIndexes.end(),
                                     individualIndexes.begin(),
                                     individualIndexes.end());
...
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::CalculateRankIndexesToSend(
    std::vector<int>& allRankIndexes, std::vector<int>& numsRankIndexes) {
    for (int i = 0, count = 0; i < allRankIndexes.size();
         i += numsRankIndexes[count], ++count) {
        int rank = allRankIndexes[i];
        rankIndexesToSend[rank].insert(
            rankIndexesToSend[rank].end(), allRankIndexes.begin() + i + 1,
...
template <typename DecisionVariableType>
std::vector<std::vector<int>>
NtMoead<DecisionVariableType>::CalculateNoduplicateIndexes(
    const std::vector<int>& neighborhoodIndexes,
    const std::vector<int>& populationSizes,
    const std::vector<std::vector<int>>& allNodeIndexes) {
    std::vector<std::vector<int>> noduplicateIndexes;
    noduplicateIndexes.reserve(parallelSize);
    for (int i = 0, start = 0; i < parallelSize;
         start += populationSizes[i], i++) {
        ...
        indexes.insert(indexes.end(), allNodeIndexes[i].begin(),
                       allNodeIndexes[i].end());
...
NtMoead<DecisionVariableType>::GenerateExternalNeighborhood(
    const std::vector<std::vector<int>>& noduplicateIndexes,
    const std::vector<std::vector<int>>& allNodeIndexes) {
    std::vector<int> noduplicateNeighborhoodIndexes;
    std::vector<int> neighborhoodSizes;
    for (int i = 0; i < noduplicateIndexes.size(); i++) {
        ...
        std::erase_if(indexes, [&](int index) {
            return std::find(nodeIndexes.begin(), nodeIndexes.end(), index) !=
                   nodeIndexes.end();
        });
...
template <typename DecisionVariableType>
std::vector<double>
NtMoead<DecisionVariableType>::GetWeightVectorsMatchingIndexes(
    std::vector<double>& weightVectors, std::vector<int>& indexes) {
...
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::InitializeExternalPopulation(
    std::vector<std::vector<double>>& receivedIndividuals) {
...
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::CalculateRanksToSent(
    const std::vector<int>& neighborhoodIndexes,
    const std::vector<int>& populationSizes,
...
NtMoead<DecisionVariableType>::ScatterPopulation(
    const std::vector<int>& ranksToSend,
    const std::set<int>& neighboringRanks) {
...
NtMoead<DecisionVariableType>::SelectParents(int index) {
...
NtMoead<DecisionVariableType>::GenerateNewIndividual(int index) {
    std::vector<Individual<DecisionVariableType>> parents =
        SelectParents(index);
...
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::UpdateNeighboringIndividuals(
    int index, Individual<DecisionVariableType>& newIndividual) {
...
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::UpdateIdealPoint(
...
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::UpdateIdealPointWithMessage(
...
NtMoead<DecisionVariableType>::CreateMessages() {
    ...
    for (auto&& [rank, indexes] : rankIndexesToSend) {
        for (auto&& index : indexes) {
            if (HasIndividual(index)) {
                ...
                dataToSend[rank].insert(dataToSend[rank].end(),
                                        receivedIndividuals[index].begin(),
...
template <typename DecisionVariableType>
std::vector<std::vector<double>>
NtMoead<DecisionVariableType>::ReceiveMessages() {
    std::vector<std::vector<double>> receiveMessages;
...
template <typename DecisionVariableType>
void NtMoead<DecisionVariableType>::UpdateWithMessage(
    ...
    for (int i = 0; i < limit; i += singleMessageSize) {
        ...
        if (!(isInternal || isExternal)) {
            receivedIndividuals[index] = std::vector<double>(
                message.begin() + i, message.begin() + i + singleMessageSize);
